...
- `assets/`: Contains all the necessary asset files used by the site.

## Features

- **Draggable Windows**: A custom implementation allowing you to move around the `rainy351.exe`, `player.exe`, and `links.exe` windows. _(and even more of them now)_
- **Window Management**:
...
3.  _Note:_ Because of browser security policies, some local files might require a local server environment (like `live-server` for VS Code or `python -m http.server`) to load correctly.
